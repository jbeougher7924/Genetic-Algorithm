...
def main():
    ...
    ProgConstant.CHROMO_LENGTH = 300
    ...
    while(True):
        ...
        while(bFound == False):
            ...
            if TargetFound:
                ...
                PrintChromo(chromo_list[foundIndex].bits,ProgConstant)
                num_elements = ParseBits(chromo_list[foundIndex].bits,buffer,ProgConstant)
                result = 0.0
                if num_elements % 2 == 0:
                    result = calcResult(buffer, num_elements)
                else:
