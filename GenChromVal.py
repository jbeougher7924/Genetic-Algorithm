...
def AssignFitness( bits, target_value,valueConst):
    ...
    if result == float(target_value):
        ...
        return 1/float(math.fabs(target_value - result))
...
def numberOfDigits(bits, valueConst):
    ...
    #print "Result: {} Length: {}".format(abs(int(result)),len(str(abs(int(result)))))
    return len(str(abs(int(result))))
...
def main():
    ...
    while(True):
        ...
        while(bFound == False):
            ...
            #    temp_chromo_list.append(ChromoTyp(child1, 0.0))
